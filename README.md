...
- 支持导出美观的 PDF，并保留代码高亮
- 支持为 `.flowdoc` 和导出的 PDF 写入文档元数据，例如文档唯一 ID、生成器版本、设备环境指纹
- 支持导出 / 导入 `.flowzip` 文档包，可把单篇文档或整个目录连同引用的图片、附件一起打包
...
- 如果不想手动整理资源，也可以直接导出一个 `.flowzip` 文档包
...
- 左侧是文档库、标签筛选、搜索区域
- 中间是编辑器
- 顶部可以新建、打开、保存、导出 PDF、打包 FlowZip、导入 FlowZip
...
- 附件会导出成图标卡片
- 视频嵌入会导出成说明卡片
- 会写入标准 PDF 文档属性，例如 `Title`、`Subject`、`Keywords`、`Creator`
- `Subject / Keywords` 中会带上 FlowDoc 的文档唯一 ID 和设备环境指纹，方便归档或追踪来源
...
- `documentId`：文档唯一 ID
- `createdByDevice` / `lastSavedByDevice`：基于当前设备环境生成的指纹字符串
- `generator`：生成该文档的 FlowDoc 应用名和版本
- `environment`：平台、架构、系统版本

说明：

- 设备指纹是哈希后的字符串，不会直接把原始主机名明文写进去
- 同一篇文档后续继续编辑时，会保留原来的 `documentId`
- 导出 PDF 时，这些信息会同步写进 PDF 元信息，方便在阅读器或文件属性里查看

---

## FlowZip 文档包 📦

FlowZip 是 FlowDoc 自己的压缩包格式，扩展名是：

```text
.flowzip
```

你可以把它理解成“文档 + 引用资源”的一键打包版，适合：
...
- 点击“打包 FlowZip”
- 然后在弹出的系统提示里选择：
  - “选择文档”：会再弹出文件选择框，让你选一篇 `.flowdoc`，然后把这篇文档以及它正文里实际引用到的图片、附件一起打包
  - “选择目录”：会再弹出目录选择框，递归扫描你选中的目录，把其中所有 `.flowdoc` 以及它们引用的资源一起打包
...
导入时资源会按当前项目的现有存放规则恢复：

- 文档会解压到你选择的目标目录
- 图片会恢复到 `images/`
- 附件会恢复到 `attachments/文档名/附件名`
- 文档正文中的资源引用会自动改写成新的相对路径

补充说明：

- 如果压缩时某些资源原本就缺失，FlowZip 仍会导出文档本体，但会在导入或导出结果里给出提示
- 这样做是为了兼容当前编辑器的资源清理逻辑，避免图片和附件引用混乱
