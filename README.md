...
- 支持导出美观的 PDF，并保留代码高亮
- 默认扫描当前用户文档库目录中的 `.flowdoc` 文件，形成文档库
- 默认会优先沿用已存在的桌面 `本地文档`；如果没有，则使用系统 `Documents/FlowDoc Library`
- 如果文档保存在别处，也可以直接“打开文档”使用；还可以通过环境变量 `FLOWDOC_LIBRARY_ROOT` 指定文档库目录
...
补充说明：
- 迁移脚本默认沿用和应用本体相同的文档库解析规则
- 默认会优先沿用已存在的桌面 `本地文档`；如果没有，则使用系统 `Documents/FlowDoc Library`
- 如果你的文档存放在别的位置，可以通过 `--documents-root <目录>` 或环境变量 `FLOWDOC_LIBRARY_ROOT` 指定扫描目录
